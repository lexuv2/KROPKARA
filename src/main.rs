        ...
        match mode.trim() {
            "p" | "P" => {
                ...
                */
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                        map2[i as usize][j as usize] = perlin_scaling(perlin((i as f64)/(100.0) + (ii as f64), (j as f64)/(100.0) + (jj as f64))) * (ZMAX - ZMININI)/1.0 + ZMININI/1.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                        map2[i as usize][j as usize] += perlin_scaling_2(perlin((i as f64)/(10.0) + (ii as f64), (j as f64)/(10.0) + (jj as f64))) * (ZMAX - ZMININI)/10.0 + ZMININI/10.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                        map2[i as usize][j as usize] += perlin_scaling_2(perlin((i as f64)/(1.0) + (ii as f64), (j as f64)/(1.0) + (jj as f64))) * (ZMAX - ZMININI)/100.0 + ZMININI/100.0;
                    }
                }
                */

                /*
                let ii = rand::thread_rng().gen_range(0..1000000);
                let jj = rand::thread_rng().gen_range(0..1000000);
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                //        map2[i as usize][j as usize] += perlin((i as f64)/(32.0) + (ii as f64), (j as f64)/(32.0) + (jj as f64)) * (ZMAX - ZMININI)/8.0 + ZMININI/8.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                //        map2[i as usize][j as usize] += perlin((i as f64)/(16.0) + (ii as f64), (j as f64)/(16.0) + (jj as f64)) * (ZMAX - ZMININI)/16.0 + ZMININI/16.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                //        map2[i as usize][j as usize] += perlin((i as f64)/(8.0) + (ii as f64), (j as f64)/(8.0) + (jj as f64)) * (ZMAX - ZMININI)/32.0 + ZMININI/32.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                //        map2[i as usize][j as usize] += perlin((i as f64)/(4.0) + (ii as f64), (j as f64)/(4.0) + (jj as f64)) * (ZMAX - ZMININI)/64.0 + ZMININI/64.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                //        map2[i as usize][j as usize] += perlin((i as f64)/(2.0) + (ii as f64), (j as f64)/(2.0) + (jj as f64)) * (ZMAX - ZMININI)/128.0 + ZMININI/128.0;
                    }
                }
                ...
                for i in 0..(XMAX+1) {
                    for j in 0..(YMAX+1) {
                //        map2[i as usize][j as usize] += perlin((i as f64)/(1.0) + (ii as f64), (j as f64)/(1.0) + (jj as f64)) * (ZMAX - ZMININI)/256.0 + ZMININI/256.0;
                    }
                }
                */
...
//                    spring3(&mut map3, xx , yy , f64::EPSILON , 8 as i64, &mut hmap, speed_preservation, ero, 0, &mut to, i+1, 0.0, &mut ts, &mut sm, range, selfbias, spring_capacity);
                    drop3_iter1(&mut map3, xx , yy , f64::EPSILON , 8 as i64, &mut hmap, speed_preservation, ero, 0, &mut to, i+1, &mut ts, &mut sm, range, selfbias, &mut lost_over_map);
                    //println!("*dies of cringe*\n");
...
fn drop3_iter1 (map: &mut Vec<Vec<f64>>, drop_x:i64, drop_y:i64, mut speed:f64, drop_dir:i64, dropmap: &mut Vec<Vec<f64>>, speed_preservation:f64, erosion:f64, life:i64, odw: &mut Vec<Vec<i64>>,moment:i64, ts:&mut Vec<Vec<f64>>, speedmap:&mut Vec<Vec<f64>>, range:i64, selfbias:f64, lost_over_map: &mut f64) {
...
/*
fn drop1 (map: &mut Vec<Vec<f64>>, ero:f64, loss:f64, hmap: &mut Vec<Vec<f64>>) {  //just go down, cut %

    let mut x:i64 = rand::thread_rng().gen_range(0..((XMAX + 1) as i64));
    let mut y:i64 = rand::thread_rng().gen_range(0..((YMAX + 1) as i64));

//    let mut map = vec![ vec![0.0; (YMAX+1) as usize] ; (XMAX+1) as usize];
    
    let mut to = vec![ vec![0; (YMAX+1) as usize] ; (XMAX+1) as usize];
    
    'fallin: loop {
        if to[x as usize][y as usize] == 0{
            to[x as usize][y as usize] = 1;
            hmap[x as usize][y as usize] += 1.0;

            let mut lowest = map[x as usize][x as usize];
            let mut xl = x;
            let mut yl = y;

            for i in (x-1)..(x+2){
                for j in (y-1)..(y+2){
                    if ( i != x ) && ( j != y ) { continue; }  //random test for bruh moment lines

                    if (i < 0) || (j < 0) || (i > XMAX as i64) || (j > YMAX as i64) {
                        continue;
                    }
                    //println!("{}, {}", i, j);
                    if map[i as usize][j as usize] < lowest {
                        lowest = map[i as usize][j as usize];
                        xl = i;
                        yl = j;
                    }
                }
            ...
            let diff = map[x as usize][y as usize] - lowest;

            map[x as usize][y as usize] -= diff * ero;
            map[xl as usize][yl as usize] += diff * ero * (1.0 - loss);
            if map[xl as usize][yl as usize] > map[x as usize][y as usize] {
                map[x as usize][y as usize] = ( map[x as usize][y as usize] + map[xl as usize][yl as usize] ) / 2.0;
                map[xl as usize][yl as usize] = map[x as usize][y as usize];
            }
...
fn ini_map (map: &mut Vec<Vec<f64>>){
    for i in 0..(XMAX+1) {
        for j in 0..(YMAX+1) {
            map[i as usize][j as usize] = rand::thread_rng().gen_range(0.0..ZMAX);
        }
    }
}
...
fn radicalize (map: &Vec<Vec<f64>>, factor: f64) -> Vec<Vec<f64>>{
    ...
    for i in 0..(XMAX+1){
        for j in 0..(YMAX+1){
            map2[i as usize][j as usize] = (0.0 as f64).max( map2[i as usize][j as usize] - inc);
            //map2[i as usize][j as usize] = ZMAX.min(map2[i as usize][j as usize]);
        }
    }
...
//    let mut map = vec![ vec![0.0; (YMAX+1) as usize] ; (XMAX+1) as usize];
    ...
    'fallin: loop {
        if to[X as usize][Y as usize] == 0{
            ...
            let mut lowest = map[X as usize][Y as usize];
            let mut XL = X;
            let mut YL = Y;
